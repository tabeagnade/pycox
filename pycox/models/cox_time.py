...
class CoxTime(models.cox_cc._CoxCCBase):
    ...
    def make_dataloader_predict(self, input, batch_size, shuffle=False, num_workers=0):
        ...
        dataloader = super().make_dataloader_predict(
            new_input, batch_size, shuffle, num_workers
        )
        return dataloader
    ...
    def compute_baseline_hazards(
        ...
        base_haz = self._compute_baseline_hazards(
            input, df, max_duration, batch_size, eval_, num_workers
        )
        if set_hazards:
    ...
    def _compute_baseline_hazards(
        ...
        def compute_expg_at_risk(ix, t):
            ...
            return (
                np.exp(
                    self.predict(
                        (sub, t), batch_size, True, eval_, num_workers=num_workers
                    )
                )
                .flatten()
                .sum()
        ...
        times = (
            df.loc[lambda x: x[self.event_col] != 0][self.duration_col]
        ...
        events = (
            df.groupby(self.duration_col)[[self.event_col]]
            .agg("sum")
            .loc[lambda x: x.index <= max_duration]
        )
        base_haz = (
            events.join(at_risk_sum, how="left", sort=True)
    ...
    def _predict_cumulative_hazards(
        ...
        def expg_at_time(t):
            ...
            return np.exp(
                self.predict(
                    (input, t), batch_size, True, eval_, num_workers=num_workers
                )
            ).flatten()

        if tt.utils.is_dl(input):
            raise NotImplementedError(
                f"Prediction with a dataloader as input is not supported "
            )
        input = tt.tuplefy(input)
        max_duration = np.inf if max_duration is None else max_duration
        baseline_hazards_ = baseline_hazards_.loc[lambda x: x.index <= max_duration]
        n_rows, n_cols = (
            baseline_hazards_.shape[0],
            input.lens().flatten().get_if_all_equal(),
        )
        ...
        hazards[baseline_hazards_.values == 0] = 0.0  # in case hazards are inf here
    ...
    def partial_log_likelihood(
        self, input, target, batch_size=8224, eval_=True, num_workers=0
    ):
        def expg_sum(t, i):
        ...
        pll = (
            pll.assign(_g_preds=g_preds)
            .reset_index()
            .merge(times, on=self.duration_col)
            .set_index(idx_name)
            .assign(pll=lambda x: x["_g_preds"] - np.log(x["_expg_sum"]))["pll"]
        )
...
class MixedInputMLPCoxTime(nn.Module):
    ...
    def __init__(
        ...
        output_activation = None
        output_bias = False
        self.net = tt.practical.MixedInputMLP(
            in_features,
            num_embeddings,
            embedding_dims,
            num_nodes,

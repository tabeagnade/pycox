...
class _CoxBase(models.base.SurvBase):
    ...
    def _compute_baseline_hazards(
        self, input, df, max_duration, batch_size, eval_=True, num_workers=0
    ):
    ...
    def compute_baseline_hazards(
        ...
        base_haz = self._compute_baseline_hazards(
            input, df, max_duration, batch_size, eval_=eval_, num_workers=num_workers
        )
        if set_hazards:
    ...
    def compute_baseline_cumulative_hazards(
        self,
        input=None,
        target=None,
        max_duration=None,
        ...
        if ((input is not None) or (target is not None)) and (
            baseline_hazards_ is not None
        ):
            raise ValueError(
                "'input', 'target' and 'baseline_hazards_' can not both be different from 'None'."
            )
        if baseline_hazards_ is None:
            baseline_hazards_ = self.compute_baseline_hazards(
                input,
                target,
                max_duration,
                sample,
        ...
        assert (
            baseline_hazards_.index.is_monotonic_increasing
        ), "Need index of baseline_hazards_ to be monotonic increasing, as it represents time."
        bch = baseline_hazards_.cumsum().rename("baseline_cumulative_hazards")
        if set_hazards:
    ...
    def predict_cumulative_hazards(
        ...
        return self._predict_cumulative_hazards(
            input,
            max_duration,
            batch_size,
            verbose,
    ...
    def _predict_cumulative_hazards(
        self,
        input,
        max_duration,
        batch_size,
    ...
    def predict_surv_df(
        ...
        """
        return np.exp(
            -self.predict_cumulative_hazards(
                input,
                max_duration,
                batch_size,
                verbose,
...
class _CoxPHBase(_CoxBase):
    def _compute_baseline_hazards(
        self, input, df_target, max_duration, batch_size, eval_=True, num_workers=0
    ):
    ...
    def _predict_cumulative_hazards(
        ...
        if baseline_hazards_ is self.baseline_hazards_:
            ...
            bch = self.compute_baseline_cumulative_hazards(
                set_hazards=False, baseline_hazards_=baseline_hazards_
            )
        bch = bch.loc[lambda x: x.index <= max_duration]
        expg = np.exp(
            self.predict(input, batch_size, True, eval_, num_workers=num_workers)
        ).reshape(1, -1)
    ...
    def partial_log_likelihood(
        ...
        return (
            ...
            .assign(pll=lambda x: x["_g_preds"] - np.log(x["_cum_exp_g"] + eps))["pll"]
        )
